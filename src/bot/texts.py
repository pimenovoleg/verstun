...
START_TEXT = "\n".join(
    [
        ...
        "/settings — настройки верстки (для владельца)",
...
BOT_COMMANDS = [
    ...
    BotCommand(command="settings", description="Настройки верстки"),
]
...
SETTINGS_MENU_STALE = "Меню устарело. Отправь /settings заново."
SETTINGS_PRIVATE_REQUIRED = "Настройки работают только в личном чате с ботом."
...
PUBLISH_STATUS_UNKNOWN = (
    "Не смог подтвердить отправку поста. Проверь канал перед повторной подготовкой поста."
)
...
BUTTON_ENABLE_BLANK_SPACERS = "Включить пустые строки"
BUTTON_DISABLE_BLANK_SPACERS = "Выключить пустые строки"
...
def settings_text(add_blank_spacers: bool) -> str:
    status = (
        "Пустые строки между блоками включены."
        if add_blank_spacers
        else "Пустые строки между блоками выключены."
    )
    action = (
        "Если нужен более плотный пост, выключи эту настройку."
        if add_blank_spacers
        else "Если абзацы начнут склеиваться визуально, включи эту настройку обратно."
    )
    return "\n\n".join(
        [
            "Настройки верстки",
            status,
            (
                "По умолчанию бот вставляет пустые строки между соседними абзацами, "
                "а также между абзацами, заголовками и горизонтальными линиями. "
                "Так Telegram Rich Message выглядит ближе к исходному документу: "
                "текст не прилипает к заголовкам и разделителям."
            ),
            action,
        ]
    )
